...
import { DraggableRowCategory } from "@/components/drag-control";
...
export function DataTableFinanceCategories({
  ...
  const dataIds = React.useMemo<UniqueIdentifier[]>(
    () => data?.map(({ categoryId }) => categoryId) || [],
    [data],
  );
  ...
  return (
    <Tabs
      defaultValue="transactionCategories"
                ...
                {table.getRowModel().rows?.length ? (
                  ...
                  >
                    {table.getRowModel().rows.map((row) => (
                      <DraggableRowCategory key={row.id} row={row} />
                    ))}
                  </SortableContext>
                ) : (
                  <TableRow>
